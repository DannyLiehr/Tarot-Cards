// From Tarot API-- I just don't want to hit their endpoint constantly.
const cards = {
  nhits: 78,
  cards: [
    {
...
/**
 * Returns an HTML element to create a card on the Card deck.
 * @param {Object} crd
 * @param {Boolean} upright  If the card is upright.
 */
function buildCard(crd, upright) {
  return `
        <div 
            class="card p-4" 
            style="max-width:20rem;text-align:center; max-height:60rem;word-wrap:break-word;overflow-y:scroll"
        >
            <h3 class="subtitle is-2">${crd.name}</h3>
            <button class="button is-danger" onClick="deleteCard(this.parentElement)">Remove</button><br/>
            <img 
                src="https://www.sacred-texts.com/tarot/pkt/img/${
                  crd.name_short
                }.jpg" 
                alt="Tarot card for ${crd.name}" 
                style="max-width:75%; height:auto;${
                  !upright ? "transform:rotate(180deg)" : ""
                }"
                />
            <p>Meaning: ${upright ? crd.meaning_up : crd.meaning_rev}</p>
        </div>
        `;
}
const numberArr= [
...
$("#searchCards").on("click", function (event) {
  event.preventDefault();
  const searchValue = $("#card-name").val();
  // Replace any digit with its corresponding name in numberArr
  const replacedValue = searchValue.replace(/\b\d+\b/g, function(match) {
    return numberArr[parseInt(match)];
  });

  console.log(replacedValue);
  const isUpright = $("#card-orientation").val() == "upright" ? true : false;
  console.log(isUpright);
  const cardMatch = cards.cards.filter((c) => {
    return c.name.toLowerCase().includes(replacedValue.toLowerCase());
  })[0];

  if (cardMatch) {
    const newCard = buildCard(cardMatch, isUpright);
    $("#card-deck").append(newCard); // Use the correct selector
  } else {
    console.log("No card found matching:", replacedValue);
